通过硬件收集语音，服务端处理-----》前端 websocket 长连接获取对话展示（这里不管是提问方和回答方都现实的文字都是后端实时通信给我的，只不过输入方是通过硬件麦克风连接到后端并非前端语音识别）。前端用 vue3 实现。我个人感觉就是这个对话的形式对于前端来说才是重点？页面布局可能就是 4 和 6 划分， 4 数字人（后端写的流 可以集成到 web 页面吗？）和 6 对话区域。帮我整理下我的思路（用到技术栈）先不用回答我解决方案。
